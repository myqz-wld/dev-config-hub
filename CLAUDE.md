...
- Treat the repository root as the base for all recorded in-project paths. Use repository-relative paths and never persist a repository/worktree absolute prefix, home-directory path, or username. For known external tools/config, use portable environment-variable references such as `$HOME` or `$CODEX_HOME`; otherwise use stable relative sibling references or a neutral logical label. Preserve product-required documentation for environment variables that intentionally hold absolute paths.
