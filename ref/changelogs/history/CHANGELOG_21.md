...
#### G1 (commit 3f21f82) — secrets-dedup 算法核心
...
#### G2 (commit 72a95c8) — backup/restore tmpDir 泄漏 + partial restore
...
#### G3 (commit a84c3fb) — backup-restore 数据正确性
...
#### G4 (commit 756cc0b) — Rust 安全加固 + 性能
...
#### G5a (commit 754c2f6) — UI secret state hygiene + 状态机 + 共用组件
...
#### G5b (commit 17fd4e9) — D 批 INFO 顺手 + secrets-fill error 不含 secret 值验证

#### G6 (commit bdb38ed / 5f56db8) — 拆模块(架构债)
...
#### G7 (commit 7c6cc50) — listBackups 并发池上限 8
...
#### G8 (commit 3fef39d) — A 批 secrets-dedup R2 HIGH×4 + MED×2 + INFO×2
...
#### G9 (commit dfd4527) — B 批 backup/restore R2 HIGH×2 + MED×3 + INFO/LOW
...
#### G10 (commit 7c354e2) — C 批 Rust 安全 R2 HIGH×2 + MED×3 + LOW×2 + INFO×1
...
#### G11 (commit 263e0ea) — D 批 UI R2 MED×3 + LOW×4 + INFO×1 + RestoreBackupModal 拆件
...
#### G12 (commit d0e8182) — D 批 5 个 invariant test (D-MED-3)
...
|---|---|---|
| 基线 (c68556b) | 339 pass | 32 pass |
| R1 末态 (7c6cc50) | 346 pass (+7) | 37 pass (+5) |
| R2 末态 (d0e8182) | **412 pass (+66)** | **40 pass (+3)** |
