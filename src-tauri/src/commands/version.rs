...
//! 50ms 循环），冻 webview 直到所有 3 个 version 一次性返回。loadAllVersions 跑
//! Promise.all × 3 直接放大效应。AP-19 / CHANGELOG_17 同根问题。
//!
...
//!
//! 防御:input domain 收紧到 3 个 enum value;后端按 enum 拼**固定**命令字符串(后端
//! 完全控制,前端没法注入)。攻击面从「任意 string」收缩到「3 个 enum value」。
//!
//! **TS 端契约**: `tool` 字段 Zsh / Claude / Codex 单词 lowercase。
...
/// **REVIEW_9 C-HIGH-1**: 3 个工具 enum 替代任意 string 命令。后端按 enum 拼固定
...
/// loadAllVersions Promise.all × 3 让本函数被并发调,旧 `Regex::new(...).unwrap()` 每次重
/// 编译,虽然 regex-lite 编译很快但 3× 串起来是 N×100us ~ 1ms 浪费。
...
fn get_tool_version_inner(command: &str) -> String {
    ...
    // 后台进程继承 stdio pipe FD → loadAllConfigs Promise.all × 3 全踩 → App 首屏 / focus
