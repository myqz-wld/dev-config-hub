...
- **EnterWorktree 工具要求 process cwd 在 git repo 内** —— session#1 cwd 是 `/Users/example/Repository/personal`（非 git repo），导致 `EnterWorktree(path:)` 直接 reject。session#2/#3/#4 通过 hand_off_session plan-driven mode default cwd = mainRepo (`.`)，**就是 git repo 根**，EnterWorktree 立刻可用
